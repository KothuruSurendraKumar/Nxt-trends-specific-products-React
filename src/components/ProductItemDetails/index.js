...
class ProductItemDetails extends Component {
  ...
  getProducts = async () => {
    ...
    this.setState({apiStatus: apiResponseConstants.inProgress})
    ...
    if (response.ok) {
      ...
      const similarDetails = fetchData.similar_products.map(
        eachSimilarProduct => this.getFormattedData(eachSimilarProduct),
      )
      this.setState({
        productData: updatedData,
        similarProductsData: similarDetails,
        apiStatus: apiResponseConstants.success,
      })
    }
    if (response.status === 404) {
      this.setState({apiStatus: apiResponseConstants.failure})
    }
  }
